@extends("frontend.layout_home")
@section("do-du-lieu-vao-layout")
@include("frontend.slide")

	<!-- product -->
        <div class="product">
            <!-- Best seller -->
            <div  class="best-seller">
                <ul style="margin-left: 2px; text-align: center;"><h3 style="width: 200px;">Best Seller
                </h3 >
                    @php
                        // Gọi hàm trong controller để lấy kết quả. Do hàm hotProducts là hàm static nên có thể truy cập từ tên class mà không cần khởi tạo đối tượng
                        $hotProducts = \App\Http\Controllers\Frontend\HomeController::hotProducts();
                    @endphp
                    @foreach($hotProducts as $row)
                    <li>
                        <img style="width: 80px; margin-top: 10px" src="{{ asset('upload/products/'.$row->photo) }}" alt="">
                        <a style="width: 100px" href="{{ url('products/detail/'.$row->id) }}">{{ $row->name }}
                            <p class="p1">{{ number_format($row->price - ($row->price * $row->discount)/100) }}đ</p>
                            <p class="p3">{{ number_format($row->price) }}đ</p>
                        </a>
                    </li>
                    @endforeach
                </ul>

            </div>
            <div style="margin-left: -30px; margin-top: 100px;" class="support">
                <ul>
                    <h3  style="text-align: center;">Hỗ trợ trực tuyến</h3>
                    <br>
                    <div>
                        <h2>Chat 24/7</h2>
                        <div class="logo-fb">
                            <i class="fa-brands fa-square-facebook fa-2xl" style="color: #3B5998;"></i>
                        </div>
                        <li class="box-chat"><a href="" target="_blank">Hotline: 0366 936 128</a></li>
                    </div>
                </ul>
            </div>

            <div style="margin-left: -30px; margin-top: 100px;" class="blog">
                <div class="blog-title">
                    <ul><h3  style="text-align: center;">News</h3>
                         @php
                            $news = \App\Http\Controllers\Frontend\HomeController::hotNews();
                        @endphp
                        @foreach($news as $row)
                        <li style="border: none; height: 350px; overflow: hidden;">
                            <img style="width: 215px;" src="{{ asset('upload/news/'.$row->photo) }}" alt="{{ $row->name }}" title="{{ $row->name }}">
                            <a style="color: orange;" href="{{ url('news/detail/'.$row->id) }}">{{ $row->name }}</a>
                            <br>
                            <a href="{{ url('news/detail/'.$row->id) }}" class="blog-detail"></a>
                                <!-- Liên quan đến ck editor dùng tag sau -->
                                {!! $row->description !!}
                            </li>
                            <hr>
                            @endforeach

                        </ul>
                </div>
            </div>
            <!-- /Best seller -->

            <!-- Discout -->
            <div class="discout">
                <!-- Categories 2 -->
                <div style="margin-left: 10px; margin-top: 25px;" class="cate1">
                    @php
                    $categories = \App\Http\Controllers\Frontend\HomeController::getCategories();
                    @endphp
                    @foreach($categories as $rowCategory)
                    <h3 style="border-bottom: 3px solid orange; padding-bottom: 5px; margin-top: 30px; font-size: 20px; text-align: center">
                        {{ $rowCategory->name }}
                    </h3>
                    <ul>
                        @php
                        $products = \App\Http\Controllers\Frontend\HomeController::getProductsInCategory($rowCategory->id);
                        @endphp
                        @foreach($products as $row)
                        <li style="height: 260px">
                            <div class="tag">-{{ $row->discount }}%</div>
                            <img style="width: 140px; margin-top: -30px;" src="{{ asset('upload/products/'.$row->photo) }}" alt="">
                            <a class="link-product" href="{{ url('products/detail/'.$row->id) }}">
                                <h4>{{ $row->name }}</h4>
                                <p class="p1">{{ number_format($row->price - ($row->price * $row->discount)/100) }}đ</p>
                                <p class="p3">{{ number_format($row->price) }}đ</p><br>
                                <div style="border: 1px solid;border-radius: 999px; width: 80px; margin: 0 auto; margin-top: 10px; text-decoration: unset; background-color: orangered;color: white;" class="p3">Sale: {{ number_format($row->discount) }}%</div>
                                <a href="{{ asset('cart/buy/'.$row->id) }}"><input class="add-to-cart" type="button" submit="" value="Add to cart"></a>
                                <button style="background-color: white; color: orange; border: 1px solid orange" class="add-to-favorites add-to-cart"
                                        data-product-id="{{ $row->id }}"
                                        data-product-name="{{ $row->name }}"
                                        data-product-price="{{ number_format($row->price - ($row->price * $row->discount)/100) }}"
                                        data-product-photo="{{ $row->photo }}"
                                >Add to favorites
                                </button>
                            </a>
                        </li>
                            <script>
                                $(document).ready(function() {
                                    // var isFirstProductAdded = false;

                                    $('.add-to-favorites').on('click', function() {
                                        // if (!isFirstProductAdded) {
                                            var productId = $(this).data('product-id');
                                            var productName = $(this).data('product-name');
                                            var productPrice = $(this).data('product-price');
                                            var productPhoto = $(this).data('product-photo');

                                            // Tạo object chứa thông tin sản phẩm
                                            var productInfo = {
                                                id: productId,
                                                name: productName,
                                                price: productPrice,
                                                photo: productPhoto
                                            };

                                            // Lưu productInfo đầu tiên vào local storage
                                            localStorage.setItem('favorites', JSON.stringify(productInfo));
                                            console.log('Đã thêm vào danh sách yêu thích:', productInfo);

                                        //     isFirstProductAdded = true;
                                        // } else {
                                        //     console.log('Chỉ lưu sản phẩm đầu tiên vào danh sách yêu thích.');
                                        // }
                                    });
                                });
                            </script>

                            <style type="text/css">
                            .cate1 ul li a:hover{
                                color: orange;
                            }
                            .cate1 ul li p{
                                display: inline;
                                margin-left: 10px;
                            }
                            .cate1 ul li{
                                border: 1px solid #ccc;
                                border-radius: 5px;
                            }
                            .cate1 ul li:hover{
                                border: 1px solid orange;
                            }
                            .cate1 ul li .link-product{
                                width: 170px;
                            }
                            .discout ul li a{
                                position: unset;
                            }
                            .add-to-cart{
                                margin-top: 10px;
                                height: 30px;
                                width: 180px;
                                border: #fff 1px solid;
                                background: orange;
                                color: #fff;
                                font-size: 16px;
                                border-radius: 8px;
                                cursor: pointer;
                            }
                            .add-to-cart:hover{
                                border: orange 1px solid;
                                background: #fff;
                                color: orange;
                            }
                        </style>
                        @endforeach
                    </ul>
                    @endforeach
                </div>
                <!-- Banner cuối -->
                <div class="sub-banner-next">
                    <img src="frontend/images/test_sub_banner3.webp" alt="">
                </div>
                <!-- /Banner cuối -->

                <!-- /Categories 2 -->

            </div>
        </div>
        <!-- /Product -->
@endsection
